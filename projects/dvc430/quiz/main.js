require.config({
    paths: {
        'jquery': 'https://cdn.ons.gov.uk/vendor/jquery/1.12.4/jquery.min',
		'jqueryui': 'js/jqueryui.min',
        'd3js': 'https://cdn.ons.gov.uk/vendor/d3/3.5.17/d3.min',
        'modernizr' : 'js/modernizr',
		'TouchPunch': 'js/jquery.ui.touch-punch.min',
		'xdomain': 'js/xdomain',
		'jquerycsv':'js/jquery.csv.min',
		'pym':'https://cdn.ons.gov.uk/vendor/pym/0.4.2/pym.min'
    },
    shim: {
        'jqueryui': {
			   deps: ['jquery']
...
function buildQuiz(area){
	...
	$("#submitBtn").show();
	$("#nextBtn").show();
	$("#disclaimer").text("*Numbers are based on survey estimates");
...
//			
            ...
            //starting question index, if there's no data for Q1, jump to Q2
            if(dvc.areaData[0].pernonuk == "."){
				dvc.questionIndex=1;
				enableSlider();
...
//                });
            ...
            $("#quizTitle").text(function()  {
              return "What is migration like in "+dvc.areaData[0].AREANM+"?";  
            });
			
            
            $("#quizQuestion").html(function()  {
                        return dvc.questionno +"/" + outof + " - " + dvc.question;  
            });
            $("#quizSubtext").text(function()  {
				return dvc.subtext;
			});
            $("#questionHint").text("Slide to guess");
			$("#test").empty();
            
        //create slider
		
        d3.select('#sliderVal').style("width", '96%').append("div").attr("id","slider");
            
        $('#slider').slider({range: 'min', min:dvc.minVal, max:dvc.maxVal, value:0, step:1, slide:function(event,ui){        
            //colour in icons
            if (dvc.render=="person"||dvc.render=="baby")   {
        ...
        $("#nextBtn").hide()
            .click(function( event ) {
				...
				//If you're from the Isle of Sciliy and don't have birth from non-uk mom data skip Q3
				if(dvc.areaData[0].birthfromnonukmoms == "."){
					dvc.questionIndex=3;
					enableSlider();
					$("#actualReveal").css("width","0%");
					$("#natReveal").css("width","0%");
                ...
                if(dvc.questionIndex<=dvc.config.config.questions.length-1)    {
                    ...
                    if (dvc.render=="person"||dvc.render=="baby") {
						...
						$("#bar").hide();
						if(quizwidth < 700) {
							d3.select('#sliderVal').style("width", '96%')
						}
						else {
							d3.select('#sliderVal').style("width", '79%')
						};
						
					} else if(dvc.render=="bar"){
						d3.select('#sliderVal').style("width", '96%')
						$("#picto").hide();
						$("#pictoText").hide();
						$("#bar").show();
...
//                }
				...
				// this bit of code, check if it's gone past question 2, removes all the icons for the person question and redraws babies
				if(dvc.questionIndex==dvc.config.config.questions.length-2){
					d3.select("#pictoGram").selectAll("*").remove();
					
					d3.select("#svgpicto").attr("viewBox","2.9 0 103 43");
					setSize();
					pymChild.sendHeight()
				...
				// get rid of the extra words on the second question
				if(dvc.questionIndex==dvc.config.config.questions.length-2) {
					$("#ExtraWords").empty();
				}

                // this bit checks if it's Q3&4 and removes the estimate asterisk
				if(dvc.questionIndex>=2) {
					$("#disclaimer").empty();
					$("#actualLabel").empty();
					$("#actualLabel").text("Actual");
					$("#pictoActualLabel").empty();
					$("#pictoActualLabel").text("Actual");
				}		
...
//				},800);
			...
			//outline the icons associated with user guess
		...
		// pull out some extra info on the second question and bit it into the ExtraWords div
			if(dvc.questionIndex==dvc.config.config.questions.length-3) {
					//$("#submitDiv").empty();
					setTimeout(function(){
						d3.select("#ExtraWords").text("People leave the UK as well as enter. It is estimated that " + dvc.format(dvc.areaData[0].Outflow) + " people left "+ dvc.areaData[0].AREANM + " intending to live outside the UK for more than a year.")
						pymChild.sendHeight()
					},1600);
			}
			
			//For the last question after users push submit remove the buttons so people can't see a next button
			if(dvc.questionIndex==dvc.config.config.questions.length-1) {
			$("#submitDiv").empty();
			}
			
			//If there's no data for NI number registrations i.e Northern Ireland, end the quiz.
			if(dvc.areaData[0].ninoreg == "." && dvc.questionIndex==2){
			$("#submitDiv").empty();
			$("#questionHint").empty();
			}
		...
		//reference icon for babies
		svgDocP.append("defs")
            .append("g")
            .attr("id","babyIcon")
			.attr("transform","scale(1.5)")
            .append("path")
				.attr("d","M74,27.4l-10.8-6.9l0,0l0,0l0,0l0,0c-1.5-0.9-2.8-1-3.8-0.6l0,0c-0.5,0.2-0.9,0.4-1.4,0.7c-1,0.5-2,0.8-3.1,1v14.3c0,0.4-0.3,0.8-0.7,1c-0.1,0-0.2,0.1-0.3,0.1s-0.2,0-0.3-0.1c-0.4-0.1-0.7-0.5-0.7-1V21.6c-1.1-0.1-2.2-0.4-3.1-1c-0.5-0.3-0.9-0.5-1.4-0.7l0,0c-1-0.4-2.4-0.3-3.8,0.6l0,0l0,0l0,0l0,0l-10.8,6.6c-2.3,1.3-2.7,3.4-1.7,5.1s2.9,2.4,5.2,1l6.7-3.3c-0.7,5-0.8,6.6-0.7,9.2c0,0.4,0,1.7,0,1.7c-2,1.1-4.4,3.9-5.2,5c-3.6,4.1-2.7,7.7-2.1,9.1c1.1,2.4,2.4,4.4,2.5,4.5c0.8,1.1,2.1,1.8,3.4,1.8c0.8,0,1.6-0.3,2.3-0.7c1.8-1.2,2.1-3.8,0.8-5.7c0,0-0.8-1.1-1.6-2.7c0.9-0.7,2.7-1.7,4.1-2.5c0.6-0.3,1.7-1,2.3-1.3c0,0.1,1.8,1,4.4,1l0,0c0.1,0,0.1,0,0.2,0c0.1,0,0.1,0,0.2,0l0,0c2.8,0,4.1-1,4-1c0.4,0.3,1.6,1,2.2,1.2c1.4,0.8,3.2,1.8,4.1,2.5c-0.8,1.6-1.6,2.7-1.6,2.7c-1.2,1.8-1,4.4,0.8,5.7c0.7,0.4,1.5,0.7,2.3,0.7c1.3,0,2.6-0.6,3.4-1.8c0.1-0.1,1.3-2,2.5-4.5c0.6-1.4,1.6-5-2-9.2c-0.9-1-3.4-3.8-5.3-4.9c0,0,0-1.1,0-1.5c0.1-2.6,0-4.5-0.7-9.5l6.7,3.6c2.4,1.4,4.3,0.7,5.2-1C76.7,30.8,76.2,28.7,74,27.4z M44.400000000000006,10.3a9.3,9.3 0 1,0 18.6,0a9.3,9.3 0 1,0 -18.6,0");
						...
						if(quizwidth < 700)
							{
								...
								d3.select('#sliderVal').style("width", '80%')
							 }
			} else if (dvc.render=="bar") {
						d3.select('#sliderVal').style("width", '96%');
						$("#picto").hide();
...
//
//            $("#quizTitle").text(function()  {
//              return "What is migration like in "+dvc.areaName+"?";  
//            });
//			
//            $("#quizQuestion").html(function()  {
