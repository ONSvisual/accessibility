...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
		...
		function setRates(thisdata){
			...
			thisdata.forEach(function(d) { rateById[d.AREACD] = +eval("d." + variables[a]); areaById[d.AREACD] = d.AREANM});
		...
		function defineBreaks(data){
			...
			var values =  thisdata.map(function(d) { return +eval("d." + variables[a]); }).filter(function(d) {return !isNaN(d)}).sort(d3.ascending);
			...
			if(config.ons.breaks =="jenks" || config.ons.breaks =="equal") {
					...
					for (var column in data[0]) {
						...
						values[column] =  thisdata.map(function(d) { return +eval("d." + column); }).filter(function(d) {return !isNaN(d)}).sort(d3.ascending);
		...
		function setAxisVal(code) {
			if(mobile == false) {
				...
				d3.select("#currVal")
					...
					.attr("y", function(){if(!isNaN(rateById[code])) {return y(rateById[code])-25} else{return y(midpoint)}});
		...
		function createKey(config){
			...
			if(mobile == false) {
				...
				var xAxisTime = d3.axisBottom(x)
					.tickSize(5)
					.tickValues([dvc.timepoints[0], dvc.timepoints[4], dvc.timepoints[8], dvc.timepoints[12],dvc.timepoints[dvc.timepoints.length-1]])
					.tickFormat(d3.format(".0f"));

				var linedata2 = [["2001",1.63],["2002",1.64],["2003",1.72],["2004",1.77],["2005",1.77],["2006",1.83],["2007",1.88],["2008",1.92],["2009",1.90],["2010",1.94],["2011",1.93],["2012",1.94],["2013",1.85],["2014",1.83],["2015",1.82],["2016",1.81],["2017",1.76],["2018",1.70]];
				...
				line2 = d3.line()
					.defined(function(linedata2) { return !isNaN(linedata2[1]); })
				...
				svgkey.append("g")
					...
					svgkey.append("text")
							...
							.attr("x", function(d){ return x(linedata2[linedata2.length-1][0])+15})
							.attr("y", function(d) { return y(linedata2[linedata2.length-1][1])-5})
							.attr("font-size", "12px")
							.style("opacity",0.3)
							.attr("fill","#666")
				...
				svgkey.append("g").attr("id","timeaxis")
					.attr("transform", "translate(60," + (10 + keyheight) + ")")
					.attr("font-weight","400")
					.style("font-family","'Open Sans'")
					.style("font-size","12px")
					.call(xAxisTime)
				...
				d3.select("#line2").attr("display","block")
				//
				// g.append("line")
				// 	.attr("id", "currLine")
				// 	.attr("y1", y(10))
