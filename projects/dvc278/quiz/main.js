                    ...
                    $.getJSON("data/config.json", function (data) {
                        ...
                        d3.csv("data/data.csv", function (data) {
                            ...
                            if (dvc.areaData[0].pernonuk == ".") {
                                ...
                                $('#slider').slider({
                                    range: 'min', min: dvc.minVal, max: dvc.maxVal, value: 0, step: 1, slide: function (event, ui) {
                                        //colour in icons
                                ...
                                $('.ui-slider-handle').attr('aria-label', 'Move the slider left and right to alter your guess')
                                ...
                                $("#submitBtn").click(function (event) {
                                    ...
                                    //animate the bar
                                    if (dvc.render == "bar") {
                                        realAnswer = dvc.prefix + dvc.format(Math.round(dvc.realAnswers[dvc.questionIndex]))
                                        myAnswer = $('#guessValue').text()
                                        setTimeout(function () {
                                            $("#actualValueText").text(realAnswer);
                                        }, 800)
                                    ...
                                    if (dvc.render == "person") {
                                        ...
                                        myAnswer = Math.round(dvc.userAnswers[dvc.questionIndex]);
                                        realAnswer = Math.round(dvc.realAnswers[dvc.questionIndex]);
                                    ...
                                    // announceUpdate
                                    $('#guess1Container').attr('aria-live', 'off')
                                    var announceText = [
                                        'For the question: ', $('#quizQuestion').text(), ' You guessed: ',
                                        myAnswer, '.',
                                        ' The real answer is: ', realAnswer, '.'
                                    ].join('')
                                    $('#accessibilityInfo').text(announceText)

                                    if (dvc.questionIndex == dvc.config.config.questions.length - 1) {
                                        $("#submitDiv").empty();
                                        $("#submitDiv").attr('aria-live', 'polite');
                                        setTimeout(function () {
