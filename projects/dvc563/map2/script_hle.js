...
//      }
 ...
 function setMapAxisVal(code) { //console.log("currVal "+ code, rateById[code]);
...
// class and css as above
        svgkey.append("g").attr("id", "timeaxis")
          ...
          .call(xAxisTime)
			...
			d3.select("#timeaxis").selectAll("text").each(function(d,i){
							 d3.select(this).text(d + "-" + (+d - 1998))
						})
...
function updateChart(code) { //console.log("updateChart"); //console.log(rankdata);
   ...
   if (chartDrawn == false) { // draw first line if not present
        ...
        svgkey.append("g").attr("id", "chartgroup")
          .attr("transform", "translate(45,30)")
          .append("path")
          .attr("id", "line1")
          .style("opacity", 1)
