...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
		...
		data.forEach(function(d) { rateById[d.AREACD] = eval("d." + dvc.varname); areaById[d.AREACD] = d.AREANM; popgrowth[d.AREACD] = d.popgrowth; empgrowth[d.AREACD] = d.empgrowth});
      ...
      legend.append('b')
        ...
        .style('background-color', function(d, i) {
          return dvc.colour_palette[i]
        })
      legend.append('label')
        .html(function(d, i) {
          return dvc.legendEntries[i];
        });
				...
				map.addLayer({
					...
					'paint': {
						'fill-color': [
							'match',
							['get', 'fill'],
							'Declining Population, Declining Employment', dvc.colour_palette[0],
							'Population Growth, Declining Employment', dvc.colour_palette[1],
							'Population Growth, Employment Growth', dvc.colour_palette[2],
							'Declining Population, Employment Growth', dvc.colour_palette[3],
							/* other */ dvc.colour_palette[4]
						],
						'fill-opacity': 1,
		...
		function onMove(e) {
            ...
            firsthover = false;
				...
				if(newAREACD != oldAREACD) {
					...
					setAxisVal(e.features[0].properties.AREANM, e.features[0].properties.popgrowth, e.features[0].properties.empgrowth );
				}
		};
    ...
    //     })
		...
		function resetZoom() {
		 ...
		 function setAxisVal(name, pop, emp) {

			 d3.select("#keydiv").html(name + "<br>Population growth: " + displayformat(pop) + "%<br>Employment growth: " + displayformat(emp) + "%").style("padding","5px 10px");
		...
		//
		 ...
		 function hideaxisVal() {
			 ...
			 d3.select("#keydiv").text("").style("padding","0px 0px")
		// 	d3.select("#currLine")
		// 		.style("opacity",0)
		//
