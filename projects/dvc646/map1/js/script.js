...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
      ...
      legend.append('label')
		...
		areas.features.map(function(d,i) {
			//console.log(rateById[d.properties.AREACD])
				...
				map.addLayer({
					...
					'paint': {
						'fill-color': [
							'match',
							['get', 'fill'],
							'Higher Deprivation Working', dvc.colour_palette[0],
							'Higher Deprivation Residential', dvc.colour_palette[1],
							'Lower Deprivation Working', dvc.colour_palette[2],
							'Lower Deprivation Residential', dvc.colour_palette[3],
		...
		function onMove(e) {
            ...
            firsthover = false;
				...
				if(newAREACD != oldAREACD) {
					...
					setAxisVal(e.features[0].properties.AREANM);
				}
		};
    ...
    //     })
		...
		function resetZoom() {
		 ...
		 function setAxisVal(name) {

			 d3.select("#keydiv").text(name).style("padding","5px 10px");
