		...
		*/
		...
		function drawGraphic()
		{
			...
			for (var column in graphic_data[0]) {
				...
				dots = graphic_data.map(function(d,i) {
					...
					return {
						...
						'1' : +d["Total imports from UK"],
						'2' : +d["Great Circle Distance from London"],
						'3' : +d["Total imports from UK"],
						'4' : +d.var4,
						'5' : +d.var5,
						'value' : +d.value
					};
			...
			svg.append("text")
				...
				.text("Imports from UK in £ billion");
			...
			if ( graphic.width() > dvc.config.optional.mobileBreakpoint ) {
				...
				//if ( isFirefox == false ) {
					...
					$(element).popover({
							...
							trigger: 'manual',
							html : true,
							content: function() { if(d[dvc.selectedYVariableIndex]<1){return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Import: £" + numberWithCommas(d[dvc.selectedYVariableIndex]*1000) + " million</b></span>"; }else{return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Import: £" + numberWithCommas(d[dvc.selectedYVariableIndex]) + " billion</b></span>"; }}
					});
