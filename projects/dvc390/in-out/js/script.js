			...
			for (var column in graphic_data[0]) {
				...
				dots = graphic_data.map(function(d,i) {
					...
					return {
						'group' : d.group,
						'name' : d.name,
						'0' : +d["Healthy Life Expectancy"],
						'1' : +d["Inequality within local authority"],
						'2' : +d["Healthy Life Expectancy "],
						'3' : +d["Inequality within local authority "],
						'4' : +d.var4,
						'5' : +d.var5,
						'value' : +d.value
					};
...
//
//				// redefine domains for x and y axis
//				dvc.xDomain = [ 0 , dvc.xMax ];
//				dvc.yDomain = [ 0 , dvc.yMax ];
//
//
//			} // end if ...
//			else {
...
//				dvc.yDomain = [ 0 , Math.ceil(d3.max(dvc.yData)/1)*1 ];
//
//
//			}// end else if ...
			...
			svg.append("text")
				.attr('class', 'unit')
				.attr('id', 'yAxisLabel')
				.attr("x", x(dvc.xDomain[0]) -35 )
				.attr("y", y(dvc.yDomain[1]) )
				.attr("text-anchor" , "start")
				.text("Total exports £ billion");
			...
			svg.append("p")
				...
				.attr("y", y(dvc.yDomain[0]) + margin.top + 40)
				.attr("text-anchor" , "end")
				.text("Total imports £ mililon");


			if (dvc.config.optional.vertical_line == true){
											dvc.config.optional.annotateLineX1_Y1_X2_Y2.forEach(function(d,i) {
										...
										.attr('x1',x( dvc.config.optional.annotateLineX1_Y1_X2_Y2[i][0][0]) )
										.attr('x2',x( dvc.config.optional.annotateLineX1_Y1_X2_Y2[i][1][0]) )
										.style('stroke', '#888')
										.style('stroke-width', 2)
										.style('stroke-dasharray', '5 5')
										.attr('y1',y(dvc.config.optional.annotateLineX1_Y1_X2_Y2[i][0][1]))
										.attr('y2',y(dvc.config.optional.annotateLineX1_Y1_X2_Y2[i][1][1]))
										//.attr("transform", "translate(" + dvc.Padding_left + "," + dvc.Padding_top + ")")
										.attr("transform", "translate(0," + ( dvc.Padding_top ) + ")");
								})
						}
...
//
//				// circles for mobile chart
////							svg.append("circle")
////								.attr("class", "annocirccirc" + (i))
...
//						})// end foreach
						...
						dvc.config.essential.annotationChart.forEach(function(d,i) {
							...
							svg.append("text")
								.text(dvc.config.essential.annotationBullet[i])
							...
							function insertLinebreaks() {
								...
								var el1 = dvc.config.essential.annotationBullet[i];
								var el = el1.data;
						...
						///d3.select(".annotext2").remove();
						//d3.select(".annotext3").remove();
						//d3.select(".annotext4").remove();
		...
		//_Swoopy draggy________________________________________________________________
		//______________________________________________________________________________
		...
		// 			"xVal": 74.2,
		// 		    "yVal": 6150,
		// 		    "path": "M298,82 L157,82",
		// 		    "text": "The UK's 9.9% of GDP spend ranks it among the 20 highest health care spenders of the OECD (the Organisation for Economic Cooperation and Development) nations.",
		// 		    "textOffset": [310,87]
		// 		}
		// 	]
		...
		// 	.attr("transform", "translate(" + (chart_width/2 + margin.left) + "," + (height/2 + margin.top) + ")")
		// 	.attr("class", "swoopy")
		// 	.call(swoopy);
		...
		// })
			...
			svg.selectAll(".circle")
				...
				//.attr( "r" , function( d , i ) { if (i== 32) {return 5} else {return 5}})
//				.style( "fill" , function ( d , i ) { if (i== 32) {return "#F85F73"} else {return "#2b8cbe"}})
//				.style( "stroke" , function ( d , i ) { if (i== 32) {return "#F85F73"} else {return "#2b8cbe"}})
				.attr( "r" , 5)
				.style( "fill" , function( d , i ) {// console.log("Y-X: " + (d[dvc.selectedYVariableIndex] - d[dvc.selectedXVariableIndex]) );
													if( d[dvc.selectedXVariableIndex] > d[dvc.selectedYVariableIndex] )
															{ return "#ED028C"} else
															{return "#1074BC" }
													})
				.style( "stroke" ,function( d , i ) {// console.log("Y-X: " + (d[dvc.selectedYVariableIndex] - d[dvc.selectedXVariableIndex]) );
													if( d[dvc.selectedXVariableIndex] > d[dvc.selectedYVariableIndex] )
															{ return "#ED028C"} else
															{return "#1074BC" }
													})
...
//								var y1 = event.clientY;
						...
						.on("mouseout",  function(d,i){
							...
							if ( dvc.booleanButtons.indexOf(true) == -1 ) {
								...
								d3.selectAll(".circles")
									.attr("class" , function(d,i){ return "circles " + "grp0"; })
									.attr( "fill" , function ( d , i ) { if (i== 32) {return "#F85F73"} else {return "#2b8cbe"}})
									.attr( "stroke" , function ( d , i ) { if (i== 32) {return "#F85F73"} else {return "#2b8cbe"}})
									.style( "opacity" , 1.00 )
		...
		*/
		...
		$(document).ready(function(){
			$("#nationUK").hover(function(){
			    $('#voronoi32').triggerSVGEvent('mouseover');
			    }, function(){
			    $('#voronoi32').triggerSVGEvent('mouseout');
			});
		});
			...
			if ( graphic.width() > dvc.config.optional.mobileBreakpoint ) {
				...
				//if ( isFirefox == false ) {
					...
					$(element).popover({
							...
							trigger: 'manual',
							html : true,
							content: function() { if(d[dvc.selectedXVariableIndex]<1&&d[dvc.selectedYVariableIndex]>1){
								return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Import: £" + numberWithCommas(d[dvc.selectedXVariableIndex]*1000) + " million</b></span><br><span style='font-size: 14px; text-align: center;'><b>Export: £" + numberWithCommas(d[dvc.selectedYVariableIndex]) + " billion</b></span>";
							}else if (d[dvc.selectedYVariableIndex]<1&&d[dvc.selectedXVariableIndex]>1) {
								return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Import: £" + numberWithCommas(d[dvc.selectedXVariableIndex]) + " billion</b></span><br><span style='font-size: 14px; text-align: center;'><b>Export: £" + numberWithCommas(d[dvc.selectedYVariableIndex]*1000) + " million</b></span>";
							}else if (d[dvc.selectedXVariableIndex]<1 && d[dvc.selectedYVariableIndex]<1){
								return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Import: £" + numberWithCommas(d[dvc.selectedXVariableIndex]*1000) + " million</b></span><br><span style='font-size: 14px; text-align: center;'><b>Export: £" + numberWithCommas(d[dvc.selectedYVariableIndex]*1000) + " million</b></span>";
							}else if (d[dvc.selectedXVariableIndex]>1 && d[dvc.selectedYVariableIndex]>1){
								return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Import: £" + numberWithCommas(d[dvc.selectedXVariableIndex]) + " billion</b></span><br><span style='font-size: 14px; text-align: center;'><b>Export: £" + numberWithCommas(d[dvc.selectedYVariableIndex]) + " billion</b></span>";
							}
						}
