	...
	//		myMin = Math.floor(mn);
		...
		xAxis.tickFormat(function(d,i) {
			...
			//create x axis, if y axis doesn't start at 0 drop x axis accordingly
				...
				svg.append("text")
					 .attr('class', 'unit')
					 .attr('transform',"translate(" + (margin.left + chart_width - 10) + ", -20)") //+margin.left
		...
		// gridlines
			...
			data.forEach(function(d,i) { if(i%12 == 0 || i == 0 || i == data.length-1 ){
				svg.append("line")
					.attr("class", "gridDash")
		...
		start = parseMyTime(dvc.essential.clampStart);
		 ...
		 var drag = d3.drag()
				  .on('drag', function(){
					...
					pos = d3.mouse(this); //console.log("mouse pos:"+pos);
	...
	function showMe(){
      if (typeof toDraw != "undefined") {
                  ...
                  toDraw.forEach(function(d, i){
                    data_user_line.push(formatDate(d.year)+": "+Number(d.debt));
                  })
      ...
      toDraw=[];
			...
			clipRect.transition().duration(1000).attr('transform',"translate(" + x(firstChart) + ", 0)").attr('width', x(finishChart) );

			//d3.select(".btn").attr("opacity", 1);
							
			//console.log("Orange text: "+ typeof debt,  debt);				
			//console.log(Number(myResult));
			//console.log(Number(fmat(result - (dvc.essential.difference/2))))
			//console.log(Number(fmat(+result + (dvc.essential.difference/2))));
			
	
		//console.log(yourData, d3.mean(yourData, format('defined')));
...
//		if (yourData[yourData.length-1].defined === false) { console.log("2nd to last"); var myResult = yourData[yourData.length-2].debt; }
//		else{ 
		...
		/*Exact*/		if(Number(myResult) === Number(fmat(data[data.length-1].debt)) ){
							d3.select("#textResponse").append("p").text(dvc.essential.feedback[0] + myResult + dvc.essential.feedback[6]);
        ...
        /*Too low*/		} else if (Number(myResult) <= Number(fmat(result - (dvc.essential.difference/2)))){                    
							d3.select("#textResponse").append("p").text(dvc.essential.feedback[2] + myResult + dvc.essential.feedback[4]);
							d3.select("#textResponse").append("p").html(dvc.essential.appendText);
        /*Close*/   	} else if (Number(myResult) >= Number(fmat(result - (dvc.essential.difference/2))) && Number(myResult) <= Number(fmat(+result + (dvc.essential.difference/2)))){
							...
							d3.select("#textResponse").append("p").html(dvc.essential.appendText);
        /*Too high*/	}else {
							d3.select("#textResponse").append("p").text(dvc.essential.feedback[2] + myResult + dvc.essential.feedback[3]);
							d3.select("#textResponse").append("p").html(dvc.essential.appendText);
                 		}
