...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
		...
		function onLeave() {
				...
				hideaxisVal();
				d3.select("#nodataarea").text("");
				d3.select("#vert").selectAll(".tick").style("opacity","1");
		};
		...
		function setAxisVal(code) {
			if(mobile == false) {
				...
				d3.select("#currVal")
					.text(function(){if(!isNaN(rateById[code]))  {d3.select("#nodataarea").text(""); d3.select("#vert").selectAll(".tick").style("opacity","0.2"); return displayformat(rateById[code])} else {d3.select("#nodataarea").text("Data unavailable"); d3.select("#vert").selectAll(".tick").style("opacity","1"); return ""}})
					.style("opacity",1)
					.transition()
					.duration(0)
			...
			d3.select("#currVal")
				.text(function(){if(!isNaN(rateById[code]))  {return displayformat(rateById[code])} else {d3.select("#nodataarea").text("Data unavailable"); return "Data unavailable"}})
				.style("opacity",1)
				.transition()
				.duration(0)
		...
		function createKey(config){
			...
			if(mobile == false) {
				...
				d3.select("#keydiv").append("p").attr("id","keyunit").style("margin-top","15px").style("margin-left","10px").text(dvc.varunit[b]);
				d3.select("#keydiv").append("p").attr("id","nodataarea").style("margin-top","0px").style("margin-bottom","0px").style("height","20px").style("margin-left","10px").style("font-weight","bold").style("font-size","14px").text("");
				...
				svgkey.append("text")
					.attr("transform", "translate(145,135)")
					.text("E&W average")
					.style("fill","999")
