...
/*				dataLayer.push({
		...
		if(areaselected ==true) {
			...
			d3.select("#currVal")
				.text(function(){if(!isNaN(rateById[code]))  {return displayformat(dataById[code]) + " people coming from " + areaname} else {return "No flow"}})
				.style("opacity",1)
				.transition()
				.duration(0)
			...
			d3.select("#currRect")
				.transition()
				.duration(700)
				.attr("width", function(){if(!isNaN(rateById[code])) {return x(dataById[code]) + "px"} else{return 0}})
				.style("fill", color(dataById[code]));


			d3.select("#currValRight")
				.text(function(){if(!isNaN(rateById[code]))  {return displayformat(rateById[code]) + " people going to " + areaname} else {return "No flow"}})
				.style("opacity",1)
				.transition()
				.duration(0)
			...
			d3.select("#currRectRight")
				.transition()
				.duration(700)
				.attr("width", function(){if(!isNaN(rateById[code])) {return x(rateById[code]) + "px"} else{return 0}})
