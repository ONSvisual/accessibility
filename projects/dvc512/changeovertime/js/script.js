...
pymChild.sendHeight();
		...
		function setAxisVal(code) {
			if(mobile == false) {
				...
				d3.select("#currVal")
					...
					.attr("y", function(){if(!isNaN(rateById[code])) {return y(rateById[code])-20} else{return y(midpoint)}});
		...
		function updateChart(code, selectlist) {
			...
			if(chartDrawn == false) {
				...
				selectedarea = thisdata.filter(function(d) {return d.AREACD == code});
		...
		function createKey(config, i){
			...
			if(mobile == false) {
				...
				var xAxisTime = d3.axisBottom(x)
					.tickSize(5)
					.tickValues([dvc.timepoints[0], dvc.timepoints[4],dvc.timepoints[8], dvc.timepoints[12], dvc.timepoints[16]])
					.tickFormat(legendformat);
