if (Modernizr.svg) {
  ...
  // keypress add handler on wrapper to deal with using radio buttons via keyboard
  wrapper.on('keypress', function (e) {
    
    var target = e.target
    if (
      target.classList.contains('form-group') &&
  ...
  function makeHandle() {
    ...
    // set aria label for the two types of slider (hoursWorked|spendAdjust)
    sliderHandle.each(function() {
      var labelText = ''
      var ctr = $(this).closest('.container')
      var spendTitle = ctr.find('.spendingtitle')
      if (spendTitle.length > 0) {
        labelText = spendTitle.text()
      } else {
        labelText = ctr.prev().text()
      }
      
      d3.select(this).attr('aria-label', labelText + '. Slide left and right to adjust your input value')
    })
  ...
  d3.select('#button-19').on('click', function () {
    ...
    if (isNaN(housing) === false && isNaN(transport) === false && isNaN(food) === false && isNaN(alcohol) === false && isNaN(clothing) === false && isNaN(communication) === false && isNaN(hobbies) === false && isNaN(maintenance) === false && isNaN(holidays) === false) {
      ...
      // if spending is higher than NLW budget display sliders so that users can adjust their spendings
        ...
        // holidays
        if (hobbiesPeriod === 1) {
          document.getElementById("leisure-time").innerHTML = 'year';
        }
        if (hobbiesPeriod === 12) {
          document.getElementById("leisure-time").innerHTML = 'month';
        }
        if (hobbiesPeriod === 52) {
          document.getElementById("leisure-time").innerHTML = 'week';
        }
        ...
        $('#ex7').on('change', function (ev) {
          ...
          yourSpending = housingPeriod * housing + transportPeriod * transport + foodPeriod * food + alcoholPeriod * alcohol + clothingPeriod * clothing + communicationPeriod * communication + hobbiesPeriod * hobbies + holidaysPeriod * holidays + maintenancePeriod * maintenance;
          document.getElementById("changeleisure").innerHTML = '£' + d3.format(",.0f")(ev.value.newValue);
          dispDiv.innerHTML = budgetCalculation();
        });
