...
var footer = $("#footer");
var dAreaSelect = d3.select("#occselect_chosen")
...
function drawGraphic() {
    ...
    d3.select("#footer").append("p")
        ...
        .html(dvc.visualisationTitle + " - Office of National Statistics ");
...
function plot(area) {
    ...
    for (i = 0; i < headers.length; i++) {
        ...
        d3.select("#varval_" + i)
            .text(function () {
                var fmt = d3.format(".2f")
                msg = headers[i] + ": " + fmt(linevalues[0][rangeLabel]) + dvc.essential.unitofmeasure[i]
...
function ready(error, pcon, data) {
    ...
    //set title of page
    document.title = dvc.visualisationTitle;
...
function announceUpdate(area) {
    ...
    for (i = 0; i < headers.length; i++) {
        ...
        d3.select("#varval_" + i)
            .text(function () {
                var fmt = d3.format(".2f")
                msg = headers[i].replace(/\-/g, ' to ') + ": " + fmt(linevalues[0][rangeLabel]) + dvc.essential.unitofmeasure[i]
                if (linevalues[0][rangeLabel] == "null") {
                    return headers[i];
                } else if (linevalues[0][rangeLabel] == "0") {
                    return "less than 0.01% " + headers[i];
