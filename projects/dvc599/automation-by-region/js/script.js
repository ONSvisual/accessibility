...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
		...
		data.forEach(function(d) { rateById[d.AREACD] = +eval(d["2017"]); areaById[d.AREACD] = d.AREANM});


		//Flatten data values and work out breaks
		var values =  data.map(function(d) { return +eval(d["2017"]); }).filter(function(d) {return !isNaN(d)}).sort(d3.ascending);
