...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
		...
		data.forEach(function(d) {
			rateById[d.AREACD] = +eval("d." + dvc.varname);
			areaById[d.AREACD] = d.AREANM;
			supergroupById[d.AREACD] = (d.supergroup);
		});
		...
		function setAxisVal(code) {
			...
			d3.select("#currVal")
				...
				.text("Supergroup: " + supergroupById[code])
		}
		...
		function createKey(config){
			...
			d3.select("#keydiv").append("p").attr("id","supergroup").style("margin-top","-10px").style("margin-left","10px").text("");
